...
from  app.auth.controllers.create_token  import TokenRequest
from  app.auth.controllers.create_token  import TokenResponse
...
async def test_create_token_success(monkeypatch):
    ...
    monkeypatch.setattr("api_server.create_token.User.find_one", mock_find_one)
    monkeypatch.setattr("api_server.create_token.Project.get", mock_project_get)
...
async def test_create_token_invalid_user(monkeypatch):
    async def mock_find_one(query): return None
    monkeypatch.setattr("api_server.create_token.User.find_one", mock_find_one)
